...
char input[SECRETC + 1];  /* The +1 is for '\0' */
...
{
    /* Warning: hacks can not be a valid combination of letters. */
    if(!strcmp(hack, "show"))
    {
        printf( MAGENTA "CPU: "
                RESET "%s\n", secret);
    ...
    else if(!strcmp(hack, "unix"))
    {
        ...
        #else
            puts(MAGENTA "CPU: " RESET "Sorry It's not an UNIX system. Maybe Windoze?\n");
        #endif
        return 1;
    }
